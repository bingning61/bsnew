...
`cmdCB()` 从 `Twist` 中取 `linear.x`、`linear.y`、`angular.z`，分别写入协议数据区的 X、Y、Z 速度字段，调用 `crc_byte()` 计算校验后通过 `self.serial.write(output)` 发送。底盘桥还发布 `odom`、`battery`，可选发布 IMU 和超声波数据。
